...
export default function MisSolicitudesPage() {
  ...
  const loadSolicitudes = async () => {
    const { data } = await supabase
      .from('solicitudes')
      .select('*, productor:productor_id(nombre, telefono), residuo:residuo_id(tipo, cantidad_kg)')
  ...
  const columns = [
    {
      ...
      cell: (value: any) => (
        <div>
          ...
          <div className="text-xs text-gray-500">{value?.telefono || ''}</div>
        </div>
      ),
    },
    {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-2">Gestiona tus solicitudes de residuos</p>
      </div>
      ...
      {/* Table */}
      <Card title="Listado de Solicitudes" description="Todas tus solicitudes enviadas">
        <Table
          columns={columns}
          data={solicitudes}
          emptyMessage="No has enviado solicitudes"
        />
      </Card>
    </div>
  );
